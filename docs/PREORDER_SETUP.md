...
อัปเดต 2026-09-20 · **ระบบขึ้นใช้งานจริงแล้วที่ https://kuapapoh.com/preorder/**
...
| ตรวจสลิปกับธนาคาร | `functions/api/_slipcheck.js` | ✅ ต่อ Thunder Solution แล้ว |
...
## 3.4 · ตรวจสลิปกับธนาคารอัตโนมัติ

ต่อ **Thunder Solution API** (`https://api.thunder.in.th/v2/verify/bank`) ไว้แล้ว · ตั้งคีย์เป็น Secret ชื่อ `SLIP_VERIFY_KEY` และเลขบัญชีปลายทางที่ `SLIP_RECEIVER_ACCOUNT`

ทำงานยังไง: ลูกค้าแนบสลิป → เว็บอ่านคิวอาร์ในสลิป → ส่งเลขอ้างอิงไปถามธนาคารว่ามีรายการนี้จริงไหม เทียบ **ยอดเงิน** และ **บัญชีปลายทางต้องเป็นบัญชีของกลุ่ม**

| ผลตรวจ | ระบบทำอะไร |
|---|---|
| โอนจริง ยอดตรง บัญชีตรง | เปลี่ยนเป็น **ยืนยันยอด** อัตโนมัติ + จดประวัติว่าระบบเป็นคนตรวจ |
| ธนาคารไม่พบรายการ (สลิปปลอม) | คงสถานะ **ใหม่** + จดว่าตรวจไม่ผ่าน ให้ทีมงานดูเอง |
| ยอดไม่ตรง / โอนเข้าบัญชีอื่น | คงสถานะ **ใหม่** + บอกเหตุผลว่าไม่ตรงตรงไหน |
| โควตาหมด / ระบบตรวจล่ม | คงสถานะ **ใหม่** ทีมงานตรวจเอง ไม่ตัดสินแทน |

โควตาแพ็กฟรี 150 สลิป · เช็คยอดคงเหลือที่หน้าผู้ใช้ของ Thunder Solution

⚠️ **ต้องส่ง header `user-agent` ไปด้วยทุกครั้ง** ไม่งั้นด่านกันบอท (Cloudflare) ของเขาตีกลับ 403 `error code: 1010` ตั้งแต่ยังไม่ถึงตัว API

### ทดสอบตัวตรวจสลิปด้วยสลิปจริง

```powershell
$env:SLIP_VERIFY_KEY = "คีย์ Thunder Solution"
python scripts/test_slip.py "D:\path	o\slip.jpg" --amount 350
```

สคริปต์จะอ่านคิวอาร์ในรูปด้วยตัวอ่านตัวเดียวกับหน้าเว็บ แล้วถามธนาคารจริง บอกว่ายอดเท่าไหร่ ใครโอน เข้าบัญชีไหน · ใช้สลิปเก่าใบไหนก็ได้ ไม่ต้องสั่งของจริง (ผลจะขึ้นว่า "ไม่ใช่บัญชีของกลุ่ม" ซึ่งถูกต้อง)

### แท็บในชีต

| แท็บ | ใช้ทำอะไร |
|---|---|
| ออเดอร์จากเว็บ | ระบบเขียนให้อัตโนมัติ ห้ามแก้คอลัมน์ |
| กรอกมือ | ออเดอร์ที่มาทาง LINE หรือหน้าร้าน · กรอกจำนวนต่อไซส์ ระบบคิดยอดให้ |
| สรุปไซส์ | รวมยอดทั้งสองทาง ไว้สั่งโรงงาน · กดเมนู "พรีออเดอร์ → อัปเดตสรุปไซส์" เพื่อนับใหม่ |
